...
the 88 margins against its own Monte-Carlo standard error, inflated by the measured design effect
below: `|mean| + z*SE` is `0.0110` against a tolerance of `0.03` on `alpha`, and `0.0166` against
`0.05` on `beta`. Both hold, by a factor of 2.7 and 3.0 respectively.

**The 88 comparisons are not 88 independent readings, and the gate's variance says so.** Two
dependence sources coexist, and the smaller one is the visible one. The visible one: `alpha_hat`
and `beta_hat` are scale-invariant, so the two unconditional scales of a target return the same
fit — 44 exact fits, which float non-invariance renders as 47 distinct pairs over 88 cells, and
which alone would give `n_eff ~ 46`. The dominant one: the replicate key carries no parameter
(common random numbers, SPECS §1.4), so **one** innovation stream serves all eight configurations.
Measured on the `11 x 8` matrix of per-replicate margins, before any pooled interval is read:

|     | ρ̄ across configurations | Kish design effect | `n_eff` over 88 cells |
| --- | ----------------------- | ------------------ | --------------------- |
| `α` | `0.9611`                | `7.7275`           | `11.39`               |
| `β` | `0.8957`                | `7.2697`           | `12.11`               |

The 88 cells buy essentially the 11 independent streams the design contains. Ignoring this would
understate the standard error of the mean margin by a factor of `2.78` and `2.70`. The matrix is
rebuilt inside the script from the persisted frame, so `R13_qmle_recovery.csv` alone lets a third
party recompute both factors.

**The gate is an equivalence statement for a design reason, not because a bias test fires.** At
the design-effect-corrected variance the mean `beta` margin is `-0.004787` with a standard error
of `0.001936`, i.e. `t = -1.25`, two-sided `p = 0.235`: **a bias gate would not have fired**, at
the `0.001` level this stream's families use or at any conventional one. The reason the gate is
not a bias test is that a finite-sample bias of the quasi-likelihood estimator is a property of
the estimator and not a port error, so its null is false by construction — the empty-ringing
control §S4bis describes. The statistic is logged descriptively and gates nothing.
...
titles are bold, horizontally centred and prefixed `(A)`–`(D)` per preamble §S6, the legend names
the detector family rather than the bare label, and `δ_opt` is taken from the in-memory diagnostics
